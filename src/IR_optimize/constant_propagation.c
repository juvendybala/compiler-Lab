...
{
    if (val.is_const)
        return get_CONST(val.const_val);
    else
        return Fact_get_value_from_IR_var(fact, val.var);
}
...
{
    if (val.kind == UNDEF)
        VCALL(*fact, delete, var);
    else
        VCALL(*fact, set, var, val);
}
...
void ConstantPropagation_print_result(ConstantPropagation *t, IR_function *func)
{
    ...
    for_list(IR_block_ptr, i, func->blocks)
    {
        IR_block *blk = i->val;
        printf("=================\n");
        printf("{Block%s %p}\n", blk == func->entry ? "(Entry)" : blk == func->exit ? "(Exit)"
                                                                                    : "",
               blk);
